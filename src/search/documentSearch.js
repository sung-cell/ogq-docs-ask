...
function extractKeywordCategories(keyword) {
  ...
  [...words, keyword].forEach(word => {
    ...
    KEYWORDS.services.forEach(service => {
      ...
      if (lowerWord.includes(lowerService) || lowerService.includes(lowerWord)) {
    ...
    // 문서유형 매칭 (수정: lower 기준 + 양방향)
    KEYWORDS.docTypes.forEach(docType => {
      const lowerDocType = docType.toLowerCase();

      if (
        lowerWord.includes(lowerDocType) ||
        lowerDocType.includes(lowerWord)
      ) {
...
if (
  ...
  lowerFileName.includes(lowerCompany)
) {
      score += 25;
  ...
  categories.docTypes.forEach(docType => {
    ...
    // 2. 기본형 매칭 (계약서 → 계약)
    const baseForm = lowerDocType.replace(/서$/, '');
    if (baseForm && lowerFileName.includes(baseForm)) {
      score += 18; // 강화
  ...
  // 5. 폴더명에 문서유형 포함 +12
  categories.docTypes.forEach(docType => {
    if (lowerFolderName.includes(docType)) {
      score += 12;
  ...
  categories.services.forEach(service => {
    if (lowerFileName.includes(service.toLowerCase())) {
      score += 10;
      matchInfo.push(`서비스: ${service}`);
    }
    if (lowerFolderPath.includes(service.toLowerCase())) {
      score += 5;
    }
  });
  ...
  if (categories.companies.length === 0 && categories.docTypes.length === 0 && categories.services.length === 0) {
    ...
    allKeywords.forEach(keyword => {
      ...
      for (const expandedKw of expandedKeywords) {
        ...
        // 폴더명 매칭
        if (lowerFolderPath.includes(lowerExpanded)) {
          score += 8;
          if (!keywordMatched) {
            keywordMatched = true;
          }
    ...
    if (score === 0 && allKeywords.length > 0) {
      // 키워드 중 일부라도 파일명/폴더명에 있는지 체크 (동의어 포함)
      const hasAnyMatch = allKeywords.some(keyword => {
        const expandedKeywords = expandTopicKeyword(keyword);
        return expandedKeywords.some(expandedKw => {
          const lowerExpanded = expandedKw.toLowerCase();
          return lowerFileName.includes(lowerExpanded) || lowerFolderPath.includes(lowerExpanded);
        });
      });
