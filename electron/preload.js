...
contextBridge.exposeInMainWorld('csAsk', {
   ...
   * @returns {Promise<{connected: boolean, reason?: string, tokenPath?: string}>}
   */
  checkDriveConnectionStatus: async () => {
    try {
      return await ipcRenderer.invoke('check-drive-connection-status');
    } catch (error) {
      return {
        connected: false,
        reason: 'error',
  ...
  /**
   * Google Drive credentials 파일 선택
   * @returns {Promise<{success: boolean, targetPath?: string, canceled?: boolean, error?: string}>}
   */
  selectDriveCredentials: async () => {
    try {
      return await ipcRenderer.invoke('select-drive-credentials');
    } catch (error) {
      return {
        success: false,
        error: error.message
   ...
   */
  getDriveAuthUrl: async () => {
    try {
      return await ipcRenderer.invoke('get-drive-auth-url');
    } catch (error) {
      return {
        success: false,
        error: error.message
  ...
  /**
   * Google Drive token 저장
   * @param {string} authCode - 인증 코드
   * @returns {Promise<{success: boolean, error?: string}>}
   */
  saveDriveToken: async (authCode) => {
    try {
      return await ipcRenderer.invoke('save-drive-token', authCode);
    } catch (error) {
      return {
        success: false,
        error: error.message
  ...
  /**
   * 스프레드시트 URL 가져오기
   * @param {string} sheetType - 'workProgress' 또는 'meetingMinutes'
   * @returns {Promise<string|null>}
   */
  getSpreadsheetUrl: async (sheetType) => {
    try {
      return await ipcRenderer.invoke('get-spreadsheet-url', sheetType);
    } catch (error) {
      return null;
    }
  },
   ...
   */
  loadMeetingFiles: async () => {
    try {
      return await ipcRenderer.invoke('load-meeting-files');
    } catch (error) {
      throw error;
    }
  },
   ...
   */
  readMeetingFile: async (filePath) => {
    try {
      return await ipcRenderer.invoke('read-meeting-file', filePath);
    } catch (error) {
      throw error;
    }
  },
   ...
   * @param {object} data - 회의 기록 데이터
   * @returns {Promise<{success: boolean, driveLink?: string, error?: string}>}
   */
  saveMeetingLog: async (data) => {
    try {
      return await ipcRenderer.invoke('save-meeting-log', data);
    } catch (error) {
      return {
        success: false,
        error: error.message
