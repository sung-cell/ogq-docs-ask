...
import { summarizeSearchResults } from '../summary/searchResultSummarizer.js';
...
/**
 * 짧은 키워드만 입력된 경우 문서 검색으로 처리
 * 예: "스마트메시지플러스", "비즈제안서", "KT비즈"
 */
function isSimpleDocumentSearchQuery(question = '') {
  ...
  const questionIntentWords = [
    '뭐야', '무엇', '어떻게', '왜', '언제', '얼마', '알려줘', '정리', '요약', '분석', '비교', '설명'
  ];

  if (questionIntentWords.some(word => normalized.includes(word))) {
    return false;
  }
  ...
  return compact.length >= 2 && compact.length <= 30 && tokens.length <= 3;
}
...
function classifyQuestion(question) {
  ...
  // 짧은 키워드 단독 입력은 질문이 아니라 문서 검색으로 처리
  // 예: "스마트메시지플러스", "비즈제안서", "KT비즈"
  if (isSimpleDocumentSearchQuery(question)) {
    return QUESTION_TYPES.DOCUMENT_SEARCH;
  }
...
export async function handleAsk(question, events = []) {
  ...
  if (questionType === QUESTION_TYPES.UNKNOWN && isSimpleDocumentSearchQuery(question)) {
    console.log('[askHandler] UNKNOWN fallback → DOCUMENT_SEARCH:', question);
    questionType = QUESTION_TYPES.DOCUMENT_SEARCH;
  }
...
export async function handleAskForHtml(question, events = []) {
  ...
  if (questionType === QUESTION_TYPES.UNKNOWN && isSimpleDocumentSearchQuery(question)) {
    console.log('[askHandler] HTML UNKNOWN fallback → DOCUMENT_SEARCH:', question);
    questionType = QUESTION_TYPES.DOCUMENT_SEARCH;
  }
    ...
    switch (questionType) {
      ...
      case QUESTION_TYPES.DOCUMENT_SEARCH: {
        ...
        if (documents.length > 0) {
          try {
            searchSummary = await summarizeSearchResults(keyword, documents);
          } catch (err) {
            // 요약 실패는 무시 (fallback으로 기본 메시지 사용)
          }
        }
        ...
        if (documents.length === 0) {
          ...
          // 요약이 있으면 추가
          if (searchSummary) {
            answerLines.push(searchSummary);
            answerLines.push('');
          }
